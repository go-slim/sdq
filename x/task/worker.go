...
// NewWorker 创建任务工作器，注册所有已注册的任务
func NewWorker(q *sdq.Queue) *Worker {
	handlers := All()
	topics := make([]string, 0, len(handlers))
	for name, t := range handlers {
...
// NewWorkerWithTasks 创建任务工作器，只注册指定的任务
func NewWorkerWithTasks(q *sdq.Queue, taskNames ...string) *Worker {
	ctx, cancel := context.WithCancel(context.Background())
