...
func TestTaskPublish(t *testing.T) {
	...
	taskName := fmt.Sprintf("test-publish-%d", exampleCounter.Add(1))
	t.Cleanup(func() {
		unregisterTask(taskName)
	})
...
func TestTaskWithDelay(t *testing.T) {
	...
	// 注册任务（使用唯一名称支持 -count=N）
	taskName := fmt.Sprintf("test-delay-%d", exampleCounter.Add(1))
	t.Cleanup(func() {
		unregisterTask(taskName)
	})
	task := Register(taskName, &Config{
...
func TestTaskWithPriority(t *testing.T) {
	...
	// 注册任务（使用唯一名称支持 -count=N）
	taskName := fmt.Sprintf("test-priority-%d", exampleCounter.Add(1))
	t.Cleanup(func() {
		unregisterTask(taskName)
	})
	task := Register(taskName, &Config{
...
func TestTaskPublishAndReserveMultiple(t *testing.T) {
	...
	// 注册任务（使用唯一名称支持 -count=N）
	taskName := fmt.Sprintf("test-multiple-%d", exampleCounter.Add(1))
	t.Cleanup(func() {
		unregisterTask(taskName)
	})
	task := Register(taskName, &Config{
...
func Example_basicUsage() {
	...
	// 7. 启动工作器处理任务
	worker := NewWorkerWithTasks(q, processOrderName, sendNotificationName)
...
func Example_multipleWorkers() {
	...
	// 启动多个工作器并发处理
	worker := NewWorkerWithTasks(q, taskName)
...
func Example_realWorldUsage() {
	...
	// 创建并启动工作器
	worker := NewWorkerWithTasks(q, autoOrderName, orderCancelName)
