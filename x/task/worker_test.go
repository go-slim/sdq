...
// workerNoOpTicker 是一个无操作的 Ticker 实现，用于 worker 测试
type workerNoOpTicker struct{}

func (t *workerNoOpTicker) Name() string                         { return "noop" }
func (t *workerNoOpTicker) Start()                               {}
func (t *workerNoOpTicker) Stop()                                {}
func (t *workerNoOpTicker) Register(name string, _ sdq.Tickable) {}
func (t *workerNoOpTicker) Unregister(name string)               {}
func (t *workerNoOpTicker) Wakeup()                              {}
func (t *workerNoOpTicker) Stats() *sdq.TickerStats {
...
func TestNewWorker(t *testing.T) {
	...
	config.Ticker = &workerNoOpTicker{}
	...
	worker := NewWorkerWithTasks(q, "test-task-1", "test-task-2")
...
func TestWorkerStartStop(t *testing.T) {
	...
	worker := NewWorkerWithTasks(q, "test-start-stop")
...
func TestWorkerHandleJob(t *testing.T) {
	...
	worker := NewWorkerWithTasks(q, "test-handle")
	_ = worker.Start(1)
	defer worker.Stop()
...
func TestWorkerMultipleJobs(t *testing.T) {
	...
	worker := NewWorkerWithTasks(q, "test-multiple")
...
func TestWorkerHandlerError(t *testing.T) {
	...
	worker := NewWorkerWithTasks(q, "test-error")
	_ = worker.Start(1)
	defer worker.Stop()
	...
	// 检查任务被埋葬
	stats, err := sdq.NewInspector(q).StatsTopic("test-error")
	if err != nil {
...
func TestWorkerNoHandler(t *testing.T) {
	...
	// 创建 worker 但不注册任务
	worker := NewWorkerWithTasks(q, "non-existent-task")
	...
	stats, err := sdq.NewInspector(q).StatsTopic("non-existent-task")
	if err != nil {
		t.Fatalf("StatsTopic() error: %v", err)
	}
...
func TestWorkerConcurrency(t *testing.T) {
	...
	// 启动 5 个并发 worker
	worker := NewWorkerWithTasks(q, "test-concurrency")
	_ = worker.Start(5)
	defer worker.Stop()
	...
	// 验证所有任务都已完成
	stats, err := sdq.NewInspector(q).StatsTopic("test-concurrency")
	if err != nil {
		t.Fatalf("StatsTopic() error: %v", err)
	}
...
func TestWorker_PanicRecovery(t *testing.T) {
	...
	worker := NewWorkerWithTasks(q, "test-panic-recovery")
	_ = worker.Start(1)
	defer worker.Stop()
	...
	stats, err := sdq.NewInspector(q).StatsTopic("test-panic-recovery")
	if err != nil {
		t.Fatalf("StatsTopic() error: %v", err)
	}
...
func TestWorker_MultiplePanics(t *testing.T) {
	...
	worker := NewWorkerWithTasks(q, "test-multiple-panics")
	...
	// 检查统计
	stats, err := sdq.NewInspector(q).StatsTopic("test-multiple-panics")
	if err != nil {
		t.Fatalf("StatsTopic() error: %v", err)
	}
