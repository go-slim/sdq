...
func (i *Inspector) Overview() *QueueOverview {
	stats := i.inspector.Stats()
	waitingStats := i.inspector.WaitingStats()
	...
	return &QueueOverview{
		...
		StartedAt:           i.inspector.StartedAt(),
		Uptime:              time.Since(i.inspector.StartedAt()).Round(time.Second).String(),
	}
}

// ListTopics 获取所有 Topic 列表
func (i *Inspector) ListTopics() []*TopicInfo {
	topicStats := i.inspector.TopicStats()
	waitingStats := i.inspector.WaitingStats()
...
func (i *Inspector) GetTopic(name string) (*TopicInfo, error) {
	ts, err := i.inspector.StatsTopic(name)
	if err != nil {
		return nil, err
	}
...
func (i *Inspector) GetJob(ctx context.Context, id uint64, withBody bool) (*JobInfo, error) {
	meta, err := i.inspector.StatsJob(id)
	if err != nil {
		return nil, err
	}
	...
	if withBody {
		body, err := i.inspector.Storage().GetJobBody(ctx, id)
		if err == nil {
			info.BodySize = len(body)
		}
	}
...
func (i *Inspector) GetJobBody(ctx context.Context, id uint64) ([]byte, error) {
	return i.inspector.Storage().GetJobBody(ctx, id)
}
