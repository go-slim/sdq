...
func BenchmarkTimeWheelTicker_Register(b *testing.B) {
	ticker := sdq.NewTimeWheelTicker(10*time.Millisecond, 100)
	ticker.Start()
	defer ticker.Stop()
...
func BenchmarkDynamicSleepTicker_Register(b *testing.B) {
	ticker := sdq.NewDynamicSleepTicker(10*time.Millisecond, 1*time.Second)
	ticker.Start()
	defer ticker.Stop()
...
func BenchmarkTicker_WheelSlots(b *testing.B) {
	...
	for _, slots := range slotCounts {
		b.Run(fmt.Sprintf("Slots_%d", slots), func(b *testing.B) {
			ticker := sdq.NewTimeWheelTicker(10*time.Millisecond, slots)
			ticker.Start()
			defer ticker.Stop()
